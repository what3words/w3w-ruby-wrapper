...
module What3Words
  ...
  class API # rubocop:disable Metrics/ClassLength
    ...
    def autosuggest(input, params = {})
      ...
      # parameters.
      #   @:param string input: The full or partial 3 word address to obtain suggestions for. At minimum this must be the first two complete words plus 
      #     at least one character from the third word
      #   @:param int n_results: The number of AutoSuggest results to return. A maximum of 100 results can be specified, if a number greater than this is requested, 
      ...
      #     within the results set which will have a focus. Defaults to n_results. This allows you to run autosuggest with a mix of 
      #     focussed and unfocussed results, to give you a "blend" of the two.
      #   @:param string clip-to-country: Restricts autosuggest to only return results inside the countries specified by comma-separated list of uppercase ISO 3166-1 
      #     alpha-2 country codes (for example, to restrict to Belgium and the UK, use clip_to_country="GB,BE")
      ...
      #     the center of the circle, latitude and longitude, and a distance in kilometres which represents the radius. For convenience, longitude 
      #     is allowed to wrap around 180 degrees. For example 181 is equivalent to -179.
      #   @:param clip-to-polygon: Restrict autosuggest results to a polygon, specified by a list of coordinates. The polygon should be closed, i.e. the first element should be repeated as the 
      #     last element; also the list should contain at least 4 entries. The API is currently limited to accepting up to 25 pairs.
